...
void GameInfo::window_init() {
    window = glfwCreateWindow(resolution_x, resolution_y, "RCraft", NULL, NULL);
...
void gameinfo_load_config(GameInfo *GInfo_) {
    std::string settings_string = gameinfo_get_fileconfig();
    GInfo_->resolution_x = rcf_fetch_ivalue(settings_string, "resolution_x");
    GInfo_->resolution_y = rcf_fetch_ivalue(settings_string, "resolution_y");
}
